-- Bug: api_log_entry marked entries billable on INTERNAL projects. Internal
-- projects have no customer, no rate, and never appear on an invoice — the app
-- (createTimeEntryAction, time-entries/actions.ts) FORCES billable=false for
-- them, even against a forged value. The agent path skipped that and applied
-- the token's default_billable (true), so Claude-logged Shyre-dev time landed
-- billable. Mirror the app: internal -> false; otherwise explicit flag, else
-- the token default. Same 10-arg signature -> CREATE OR REPLACE preserves the
-- anon-only grant from 20260721140000.
