...
describe("computeFieldDiff", () => {
  ...
  it("with a newer entry, surfaces only fields whose values differ", () => {
    const diff = computeFieldDiff(
      { legal_name: "Robert Smith", title: "Senior" },
      { legal_name: "Robert Smith", title: "Director" },
      { labels, hiddenKeys },
    );
    expect(diff.map((d) => d.key)).toEqual(["title"]);
    expect(diff[0]!.from).toBe("Senior");
    expect(diff[0]!.to).toBe("Director");
  });
  ...
  it("treats null vs missing-key as different (the move surfaces in audit)", () => {
    const diff = computeFieldDiff(
      { ended_on: null },
      { ended_on: "2026-04-01" },
      { labels: { ended_on: "Ended" }, hiddenKeys },
    );
  ...
  it("preserves the labels-map order in the output", () => {
    const orderedLabels = { title: "Title", city: "City" };
    const diff = computeFieldDiff(
      { city: "Seattle", title: "Engineer" },
      { city: "Portland", title: "Director" },
      { labels: orderedLabels, hiddenKeys },
    );
    expect(diff.map((d) => d.key)).toEqual(["title", "city"]);
  });

  it("ignores fields present in hiddenKeys even on most-recent entry", () => {
    const diff = computeFieldDiff(
      { legal_name: "Robert", id: "noisy", business_id: "noisy" },
      null,
      {
        labels: { legal_name: "Legal name", id: "Id", business_id: "Biz" },
        hiddenKeys: new Set(["id", "business_id"]),
      },
    );
    expect(diff.map((d) => d.key)).not.toContain("id");
    expect(diff.map((d) => d.key)).not.toContain("business_id");
