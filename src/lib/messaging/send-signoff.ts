...
export interface SendSignoffEmailInput {
  teamId: string;
  userId: string | null;
  documentId: string;
  /** `signoff` = the sign-link email; `signoff_otp` = the one-time code. */
  kind: "signoff" | "signoff_otp";
  toEmail: string;
  subject: string;
  bodyHtml: string;
  bodyText: string;
...
/**
 * Send a document-sign-off email (sign link or OTP) through the same hardened
 * pipeline as proposals/invoices: team config + decrypted key → header
 * sanitation → recipient validation → from-domain allow-list → daily-cap
 * consume → outbox enqueue → provider drain. No attachments; the OTP is
 * body-only by design. Mirrors `sendProposalEmail`.
 */
export async function sendSignoffEmail(
  supabase: SupabaseClient,
  input: SendSignoffEmailInput,
): Promise<{ outboxId: string; providerMessageId: string }> {
  const cfg = await loadTeamConfig(supabase, input.teamId);
  if (!cfg) {
    throw new Error("Email is not configured for this team. Visit /settings/email.");
  }
  if (!cfg.apiKeyCipher) {
  ...
  const replyToEmail = cfg.replyToEmail ? sanitizeHeaderValue(cfg.replyToEmail) : null;
  ...
  const idempotencyKey = `${input.kind}:${input.documentId}:${dayBucket}:${randomUUID()}`;
  ...
  const message: OutboundMessage = {
    ...
    tags: {
      ...
      shyre_signoff_id: input.documentId,
