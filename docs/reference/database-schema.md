...
| Table | Purpose |
|---|---|
| `proposals` | Fixed-price quotes — the front of the funnel (draft → sent → viewed → accepted/declined → converted, plus `superseded` for replaced versions; timestamps trigger-stamped on first transition). `delivered_at` + `delivered_by_user_id` (`20260723150000`, ON DELETE SET NULL) mark a converted engagement DELIVERED — a nullable stamp, **not** a new status (mirrors `projects.closed_at`): the proposal stays `converted` (so invoicing/totals/filters keep working), guarded by `CHECK (delivered_at IS NULL OR status = 'converted')` and set/cleared by `markProposalDeliveredAction` / `reopenProposalDeliveryAction` (both write a `delivered` / `reopened` `proposal_events` row). The two columns are in the send-lock guard's `mutable[]` allow-list. UI derives the "Delivered" badge + "N of M phases delivered" readout from the stamp. Carries `team_id`, `user_id` (prepared-by), `customer_id`, `signer_contact_id` → `customer_contacts`, optional `business_id`, `proposal_number` (unique per team, from `team_settings.proposal_prefix`/`proposal_next_num`), terms (`payment_terms_days`+`label`, `deposit_type` `none\|percent\|amount` + `deposit_value`, `warranty_days`, `terms_notes`), versioning (`version_number`, `supersedes_proposal_id`, `superseded_at` — stamped by the status-timestamp trigger when a version is replaced), `accepted_total`. `deposit_invoice_id` (`20260718100000`, SAL-049) → the one deposit invoice billed for the proposal (conditional-update claimed; cleared on void/delete of that invoice). `sign_theme` (`20260717200000`, `light\|dark\|warm`, default `light`, allow-list `SIGN_THEMES`) is the color theme the client sees on the sign page + preview — pinned by the author so the document doesn't drift with each recipient's OS dark/light preference. Owner/admin-only RLS (invoice tier). |
