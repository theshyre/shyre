...
**Reference**: `/time-entries` — shared by all four views (Week / Day / Log / Table). The same `EntryTable` component renders the strip; views differ only in what they pass to `groups[]` and `showDate`.
...
3. **Destructive bulk actions** pair a labeled trigger button with a typed-confirm escalation + Undo toast per "Destructive confirmation flows" in `forms-and-buttons.md`. Summary shows the count (e.g. "3 entries"). Two flavors:
   - **Row-level destructive** (one row at a time, e.g. trash kebab) → use `<InlineDeleteRowConfirm />` from `@theshyre/ui`. Icon-only idle button — visual weight matches single-row scope.
   - **Bulk-strip destructive** (N rows at once) → labeled idle button. The icon-only treatment reads as too quiet next to "33 selected." See `InlineBulkDeleteButton` in `src/app/(dashboard)/time-entries/entry-table.tsx` for the canonical labeled-typed-confirm pattern: idle = `[Trash] Delete` with red text + neutral border (color cue, not soft-fill background); armed = "Type delete to delete N entries [input] [Delete] [X]".
4. **Reversible-but-bulk flag flips** (e.g. mark-as-billed-elsewhere) use a tier-1 inline confirm — labeled idle button, click expands to "Confirm: <action> N entries? [Yes][X]". No typed word required since Undo restores. Same labeled-button styling as the bulk-destructive trigger, neutral chrome. Pair with the standard Undo toast.
...
6. **Multiple bulk actions with disjoint row eligibility** (e.g. `/proposals`: delete on draft/superseded, mark-delivered on converted-undelivered). A row is *selectable* if **any** action can touch it (the checkbox is disabled only for fully inert rows). Compute the eligible subset **per action** from the current selection, render each action's button **only when its subset is non-empty**, label it with that subset's count, and post only those ids (the server re-checks + silently skips the rest). This keeps two actions on one strip without a selection "mode." Reference: `src/app/(dashboard)/proposals/proposals-table.tsx` (`selectedDeletableIds` / `selectedDeliverableIds`).

## Surfaces that must conform

- Pattern A: `/time-entries` (all four views — Week / Day / Log / Table — share `EntryTable`).
