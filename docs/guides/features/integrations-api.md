...
Returns an array of projects, each `{ id, name, status, is_internal, customer_id, customer_name, github_repo, jira_project_key, default_category_id, categories }`. `github_repo` (e.g. `owner/repo`) lets an agent match projects to the repo it's working in — filter the list to `github_repo == your git remote origin` to find this repo's project(s), which is how a monorepo mapped to several projects narrows the candidate set. `jira_project_key` (e.g. `AE`) is the project's **effective** Jira key (own, or inherited from its parent) and drives ticket auto-linking on `POST /api/v1/entries` (below). `categories` is the project's **effective** set (its base category set plus any project-scoped extensions) as `{ id, name, color, is_default }` — pass one of those ids as `category_id` on `POST /api/v1/entries`, or omit it to inherit the project's `default_category_id`. A **nested project with no category set of its own inherits its parent's** vocabulary and default (live — a later parent change propagates), so a deliverable under an umbrella returns the umbrella's categories rather than an empty list. `github_repo`, by contrast, is never inherited — the repo→project mapping must stay unambiguous. Rates are structurally unreachable on this surface.
...
Required: `project_id`, `start_time`/`end_time` (ISO 8601 **with timezone** — the offset may be written either `-07:00` or the bare `-0700` that `date +%z` emits; both are accepted and stored canonically, but a timezone is required — a naive `2026-07-18T14:00:00` is rejected), `description` (≥ 8 meaningful characters). Optional: `agent_label`, `session_ref`, `idempotency_key`, `billable` (overrides the token default), `category_id` (a category from the project's `categories` in `list_projects`; when omitted the project's default category is applied, so agent-logged time still lands categorized).

**Ticket auto-linking.** The first ticket reference in the `description` is linked automatically — no field to pass. A `owner/repo#123` GitHub reference links as-is; a bare `#123` resolves against the project's `github_repo`; and a Jira reference (`AE-709`) links **only when its key prefix matches the project's effective `jira_project_key`** — deliberately stricter than the in-app editor (which links any `XXX-123`), because machine-written descriptions are full of `UTF-8`/`ISO-8601`/`SHA-1`-shaped false positives. Lead your description with the ticket (e.g. `AE-709: …`) and confirm the project's key is set (see `list_projects`). The link's title is filled in lazily on first view; the structural link (provider + key + browse URL) lands at write time.
